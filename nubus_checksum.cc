...
class NuBusFormatBlock {
 public:
  ...
  static NuBusFormatBlock* ReadNuBusFormatBlock(std::ifstream& file,
    ...
    return new NuBusFormatBlock(byte_lanes, reserved, test_pattern, format,
                                revision_level, crc, rom_length,
                                directory_offset);
  }

  void WriteToFile(std::ofstream& file, std::streamsize file_length) {
...
class NuBusImage {
 public:
  static NuBusImage* ReadFromFile(std::ifstream& file,
                                  std::streamsize file_length) {
    NuBusFormatBlock* format =
        NuBusFormatBlock::ReadNuBusFormatBlock(file, file_length);
  ...
  void UpdateCRC(const uint32_t new_crc) { format_->crc_ = new_crc; }
  ...
  bool WriteToFile(std::ofstream& file, std::streamsize file_length) {
    ...
    LOG(INFO) << "Write out zero bytes at beginning " << zero_length;
 ...
 private:
  NuBusImage(NuBusFormatBlock* format, int32_t byte_len, char* contents)
      : format_(format), byte_len_(byte_len), bytes_(contents) {}
};
